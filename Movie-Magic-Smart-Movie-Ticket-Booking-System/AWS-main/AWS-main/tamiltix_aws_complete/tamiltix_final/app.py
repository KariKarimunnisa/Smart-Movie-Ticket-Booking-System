# ============================================================
#  Movie Magic - Smart Movie — Main Flask Application
#  
#  SMART MODE:
...
# ── Flask ────────────────────────────────────────────────────
app = Flask(__name__)
app.secret_key = os.getenv("SECRET_KEY", "Movie Magic-local-secret-2025")
...
TBL_USR  = os.getenv("DYNAMODB_TABLE_USERS",    "Movie Magic_users")
TBL_BKG  = os.getenv("DYNAMODB_TABLE_BOOKINGS", "Movie Magic_bookings")
    ...
    if AWS_READY and sns_client and SNS_ARN and SNS_ARN != "PASTE_YOUR_SNS_TOPIC_ARN_HERE":
        try:
            sns_client.publish(
                TopicArn = SNS_ARN,
                Message  = msg,
                Subject  = f"Movie Magic — Booking Confirmed — {booking['movie_name']} [{booking['booking_id']}]"
            )
...
def register():
    if request.method == "POST":
        ...
        flash(f"Welcome to Movie Magic, {name}! 🎉", "success")
        return redirect(url_for("home"))
...
def confirm():
    ...
    # ---- Seat Validation ----
    try:
        existing_bookings = db_get_user_bookings(session["user_email"])

        for b in existing_bookings:
            if b.get("show_key") == booking["show_key"]:
                booked_seats = b.get("seats", [])

                for seat in booking["seats"]:
                    if seat in booked_seats:
                        flash(f"Seat {seat} already booked. Choose another seat.", "error")
                        return redirect(url_for("checkout"))

    except Exception as e:
        flash(f"Seat validation error: {e}", "error")
        return redirect(url_for("checkout"))
    # Save booking
    try:
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*50}")
    print(f"  Movie Magic starting in {mode} mode")
